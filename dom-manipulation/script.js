...
async function syncQuotes() {
  ...
  if (added > 0) {
    saveQuotes();
    populateCategories();
    syncStatus.textContent = `${added} quote(s) synced from server.`;
  } else {
    syncStatus.textContent = "No new quotes found.";
  }
...
populateCategories();
loadLastQuote();
syncQuotes(); // ✅ Important: included for validati